...
public class CNFConverter {
     ...
     * . Notice the 5 steps
     * shown above will turn into 5 different methods. For the sake of
     * testing, I set them public.
    ...
    private Expression convert(Expression express)
        ...
        if (isUsed) {
            throw new IllegalStateException("The class could only be used " +
                    "once!");
        } else {
            isUsed = true;
        }
    ...
    /**
     * this is the first step that rebuild the expression tree. Use the
     * standard specified in the
     * above class. Traverse the original tree recursively and rebuild the
     * tree from that.
     *
     * @param express the original expression tree.
     */
    ...
    /**
     * This method is used to deal with pushing not operators down. Since it
     * needs an extra
     ...
     * traverse the tree thoroughly,
     * when we meet the not operator. We only need to consider these three
     * operators:
     * MultiAndOperator, MultiOrOperator, NotOperator. Handle them in a
     * seperate way. when we finish
     * the traverse, the expression tree will have all the not operators
     * pushed as downwards as they
     * could. In the method, I use two global variables: temp1 and temp2 to
    ...
    /**
     * This function mainly deals with pushing not operators down. check the
     * child. If it is not a
     * logic operator(and or or). stop at that point. Else use De Morgan law
     * to push not downwards.
     *
     * @param index the index of the children appeared in parents array.
     */
...
//                return;
    ...
    /**
     * This method serves as dealing with the third step. It is used to put
     * all the adjacent same
     * multi operators together. BFS the tree and do it node by node. In the
     * end we will get the
     * tree where all the same multi operators store in the same odd level of
     * the tree or in the
            ...
            if (express instanceof MultiAndExpression) {
                ...
                while (true) {
                    ...
                    if (index == and.size()) {
                        ...
                        /* if not, remove the child out and push the child of
                         that child
                         * in the operator, starting from the index where the
                          * child
                         * is removed. */
                ...
                /* Do the standard BFS now since all children are not and
                operators. */
                for (int i = 0; i < and.size(); i++) {
                ...
                /* Do the standard BFS now since all children are not or
                operators. */
                for (int i = 0; i < or.size(); i++) {
     ...
     * into a stack. Next, pop
     * them out and push the and operators under the or operators upwards(if
     * there are). Do this
     * level by level, which means during each level we will call the gather
     * () method to make the
     * tree uniform. When we move out of the stack. The expression tree shall
     * be in CNF form.
     */
    private void pushAndUp() {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                /* Note the child may not be an instance of multiple
                expression!. */
    ...
    /**
     * This helper function is used to deal with pushing and up: generally,
     * pop the top element out
     * of the stack, use BFS to traverse the tree and push and up. It will
     * case the expression tree
     * to have the and as the new root and multiple or as the children. Push
     * them on the queue and
     * repeat the same process until the newly generated or operator does not
     * have any and operators
     * in it(which means no elements will be added into the queue). when one
     * level is finished,
     * regroup the tree. Do this until the stack is empty, the result will be
     ...
     * Expression tree that has one
     * multiple and expression with a list of multiple or expression as the
     * child. So we need to
     * convert the multiple expression back to the binary counterparts. Note
     * the converted tree is
     * left inclined. Also I attach a parenthesis node before the or
     * expression that is attached to
