...
import preprocessing.PreprocessorManager;
...
public class Master {
    ...
    public static void executeSelect(PlainSelect select,
                                     boolean explain, int plotAtMost,
									 int plotEvery,
        ...
        if (LoggingConfig.UNNESTING_VERBOSE) {
            System.out.println("Sub-query sequence generated during " +
					"unnesting: ");
        ...
        for (int subQueryCtr = 0; subQueryCtr < nrSubQueries; ++subQueryCtr) {
            ...
            Context context =
					PreprocessorManager.getPreprocessor().process(subQueryInfo);
