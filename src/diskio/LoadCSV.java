            ...
            for (int colCtr = 0; colCtr < nrColumns; ++colCtr) {
                ...
                try {
                    switch (columnTypes[colCtr]) {
                        case ANY_TYPE:
                            throw new Exception("Cannot parse undetermined " +
                                    "type");
                        case BYTE:
                        case INT:
                            IntData intData = ((IntData) data.get(colCtr));
                            intData.data[rowCtr] = isNull ? 0 :
                                    Integer.parseInt(field);
                            break;
                        case LONG:
                            LongData longData = ((LongData) data.get(colCtr));
                            longData.data[rowCtr] = isNull ? 0 :
                                    Long.parseLong(field);
                            break;
                        case DOUBLE:
                            DoubleData doubleData =
                                    ((DoubleData) data.get(colCtr));
                            doubleData.data[rowCtr] = isNull ? 0 :
                                    Double.parseDouble(field);
                            break;
                        case STRING:
                            StringData stringData =
                                    ((StringData) data.get(colCtr));
                            stringData.data[rowCtr] = isNull ?
                                    nullRepresentation : field;
                            break;
                        case DATE:
