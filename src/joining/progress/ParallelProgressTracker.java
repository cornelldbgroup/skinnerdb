...
public class ParallelProgressTracker {
     ...
     */
    public Map<Integer, Integer>[] previousSplitTable;
    /**
     * offsets for different split tables and different threads
     */
    public int[][][] tableOffsetArray;
    /**
     * upper level thread
     */
    ...
    public ParallelProgressTracker(int nrTables, int nrThreads) {
        ...
        previousSplitTable = new HashMap[nrThreads];
        tableOffsetArray = new int[nrThreads][nrTables][nrTables];
        for (int j = 0; j < nrThreads; j++) {
            previousSplitTable[j] = new HashMap<>();
        }
    }
    ...
    public void updateProgress(JoinOrder joinOrder, int splitKey,
        ...
        tableOffsetArray[threadID][firstTable][splitTable] = Math.max(lastTreatedTuple, tableOffsetArray[threadID][firstTable][splitTable]);
        int currentOffset = Integer.MAX_VALUE;
        for (int tid = 0; tid < nrThreads; tid++) {
            currentOffset = Math.min(tableOffsetArray[tid][firstTable][splitTable], currentOffset);
        }
    ...
    public State continueFrom(JoinOrder joinOrder, int splitKey,
        ...
        for (int joinCtr = 0; joinCtr < upperLevel; ++joinCtr) {
            ...
            if (curPrefixProgress.isNull(threadID)) {
                return state;
            }
            // get a thread state
