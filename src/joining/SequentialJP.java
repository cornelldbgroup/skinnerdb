...
public class SequentialJP {
	...
	public static void process(QueryInfo query, 
		...
		*/
		...
		finalizeJoinPhase(query, context, joinOp);
		/*
	...
	/**
	 * Finalizes join phase by materializing join result
	 * and updating column references in preparation for
	 * the post-processing phase.
	 * 
	 * @param query		the query to process
	 * @param context	query processing context
	 * @param joinOp	join operator containing complete join result
	 * @throws Exception
	 */
	public static void finalizeJoinPhase(QueryInfo query, 
			Context context, MultiWayJoin joinOp) throws Exception {
		// Materialize result table
		Collection<ResultTuple> tuples = joinOp.result.getTuples();
		String targetRelName = NamingConfig.JOINED_NAME;
		Materialize.execute(tuples, query.aliasToIndex, 
