...
public class JoinTaskThread implements Callable<Double> {
    ...
    final List<JoinTaskThread> threads;
    ...
    public JoinTaskThread(AtomicBoolean isFinished, AtomicInteger budget,
                          ConcurrentLinkedQueue<int[]>[] tasks,
                          List<JoinTaskThread> threads, ParaJoin joinOp) {
...
//                reward += task.compute(tasksCache);
                int steps = taskWorker.evaluate(task, tuples);
    ...
    public boolean checkFinished() {
        for (JoinTaskThread thread: threads) {
            if (!thread.isNull) {
                return false;
            }
        }
