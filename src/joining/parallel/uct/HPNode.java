...
public class HPNode {
     ...
     */
    public final HPNode[] childNodes;
    /**
     ...
     */
    public HPNode(long roundCtr, QueryInfo query,
        ...
        childNodes = new HPNode[nrActions];
        ...
        if (nrThreads <= nrFilteredActions) {
            ...
            for (int filteredActionCtr = 0; filteredActionCtr < filteredActions.size(); ++filteredActionCtr) {
                ...
                if (childNodes[actionCtr] == null) {
                    childNodes[actionCtr] = new HPNode(roundCtr, this, table,
     ...
     */
    public HPNode(long roundCtr, HPNode parent, int joinedTable,
        ...
        if (nrThreads == 1) {
            ...
            for (int filteredActionCtr = 0; filteredActionCtr < filteredActions.size(); ++filteredActionCtr) {
                ...
                if (childNodes[actionCtr] == null) {
                    childNodes[actionCtr] = new HPNode(roundCtr, this,
     ...
     * @param joinedTable new joined table
     */
    public HPNode(long roundCtr, HPNode parent, int joinedTable, List<Set<Integer>> assignment) {
        // Count node generation
        createdIn = roundCtr;
        tid = parent.tid;
        treeLevel = parent.treeLevel + 1;
    ...
    public double sample(long roundCtr, int[] joinOrder, OldJoin oldJoin,
        ...
        if (nrActions == 0) {
            ...
            if (childNodes[action] == null && canExpand) {
                childNodes[action] = new HPNode(roundCtr, this, table, tid, tid + 1);
            }
            // evaluate via recursive invocation or via playout
            HPNode child = childNodes[action];
    ...
    public double sample(long roundCtr, int[] joinOrder, OldJoin oldJoin, List<Set<Integer>> assignment,
        ...
        if (nrActions == 0) {
            ...
            boolean canExpand = createdIn != roundCtr;
            if (childNodes[action] == null && canExpand) {
                childNodes[action] = new HPNode(roundCtr, this, table, assignment);
            }
            // evaluate via recursive invocation or via playout
            HPNode child = childNodes[action];
            double reward = (child != null) ?
    ...
    public void partitionSpace(int startThread, int endThread,
                               List<List<Set<Integer>>> actionsAssignment, HPNode node) {
