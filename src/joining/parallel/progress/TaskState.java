...
public class TaskState {
    ...
    /**
     * Last combination of tasks that
     * we have seen.
     */
     ...
     */
    public TaskState(int nrTables) {
        tupleIndices = new int[nrTables];
        tasks = new ConcurrentSkipListSet[nrTables];
        for (int table = 0; table < nrTables; table++) {
            tasks[table] = new ConcurrentSkipListSet<>();
        }
     ...
     */
    public boolean isAhead(int[] order, TaskState otherState, int prefixLength) {
     ...
     */
    public void fastForward(int[] order, TaskState otherState, int prefixLength) {
        ...
        int table = order[otherState.lastIndex];
        tasks[table] = otherState.tasks[table];
    }
