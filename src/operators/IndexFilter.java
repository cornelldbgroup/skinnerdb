...
public class IndexFilter extends PlainVisitor {
     ...
     * the predicate.
     */
    public final Deque<IntList> qualifyingRows =
            new ArrayDeque<>();
    /**
    ...
    public void visit(AndExpression and) {
        ...
        IntList rows1 = qualifyingRows.pop();
        IntList rows2 = qualifyingRows.pop();
        MutableIntList intersectedRows = IntLists.mutable.empty();
        qualifyingRows.push(intersectedRows);
        if (!rows1.isEmpty() && !rows2.isEmpty()) {
            IntIterator row1iter = rows1.intIterator();
            IntIterator row2iter = rows2.intIterator();
            ...
            while (row1 != Integer.MAX_VALUE &&
                ...
                if (row1 == row2) {
                    ...
                    row1 = row1iter.hasNext() ? row1iter.next() :
                            Integer.MAX_VALUE;
                    row2 = row2iter.hasNext() ? row2iter.next() :
                            Integer.MAX_VALUE;
                } else if (row1 < row2) {
                    row1 = row1iter.hasNext() ? row1iter.next() :
                            Integer.MAX_VALUE;
    ...
    public void visit(OrExpression or) {
        ...
        if (rows1.isEmpty()) {
            ...
            MutableIntList mergedRows = IntLists.mutable.empty();
